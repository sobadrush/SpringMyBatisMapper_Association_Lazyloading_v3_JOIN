...
public class DeptDAO_Mapper {
	...
	public List<DeptVO> getDeptsWithEmpData() {
		return mapper.getDeptsWithEmps();
	}

	public List<DeptVO> getDeptsWithEmpDataByDeptId(int deptNo) {
		return mapper.getDeptsWithEmps(deptNo);
	}
...
//		}
		...
		//【查全部 --- 關聯查詢(使用JOIN) Emp】
		List<DeptVO> list = deptDAO2.getDeptsWithEmpData();
		...
		//【查一筆 by Id --- 關聯查詢(使用JOIN) Emp】
//		List<DeptVO> list = deptDAO2.getDeptsWithEmpDataByDeptId(10);
//		for (DeptVO deptVO : list) {
//			System.err.println(deptVO.toString());
//			// 查部門下的員工：
//			Set<EmpVO> empSet = deptVO.getEmps();
